...
def load_word_dict(wordlist):
    ...
    """
    df_word = pd.read_csv(f"data/stage2/colex2cosine/controlled_edgelists/colexnet_{wordlist}.csv")
    d = dict()
...
def build_lang2geo_graph(ds="colexnet",
    ...
    outputfile = os.path.join(outputfolder, f"controlled_{ds}_geo_graph.pickle")
    ...
    if ds == "colexnet":
        ...
        d_emotion = load_word_dict("emotion")
        ...
        d_aff_conc = load_word_dict("affective_concrete")
        d_aff_abs = load_word_dict("affective_abstract")
    ...
    for lang, lang2, con, genetic, syntactic in tqdm(zip(df_lang["LANG1"], df_lang["LANG2"], df_lang["ContactDist"],
        ...
        if ds == "colexnet":
            ...
            if (l1, l2) in d_aff_conc:
                g.edges[l1, l2]["aff_conc"] = d_aff_conc[(l1, l2)]
            if (l1, l2) in d_aff_abs:
                g.edges[l1, l2]["aff_abs"] = d_aff_abs[(l1, l2)]
