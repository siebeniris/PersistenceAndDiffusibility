...
class MeasureAssociation:
    ...
    def build_records(self):
        header = ["source", "target", "weight", "target_nr", "source_nr", "normalized_weight",
                  "student_t", "chi_sq", "likelihood_ratio", "mi_like",
                  "pmi", "phi_sq", "poisson_sitrling", "jaccard", "dice"]
        rows = []
...
def main(inputfolder, outputfolder):
    for file in os.listdir(inputfolder):
        ...
        N = get_total_number_colexification(
            file=filepath)  # number of total colexificaiton
        print("total number of colexifications: ", N)
        measurer = MeasureAssociation(filepath, [], N)
        measurer.display()
        measurer = MeasureAssociation.load_data(filepath, N)
        measurer.display()
